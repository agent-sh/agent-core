...
describe('public API surface', function() {
  it('exports the same top-level functions as before, plus new helpers', function() {
    const expected = [
      ...
      'verifySlsaAttestation', 'isGhAvailable'
    ];
...
// ---------------------------------------------------------------------------
// SLSA build provenance verification (verifySlsaAttestation)
// ---------------------------------------------------------------------------

describe('verifySlsaAttestation', function() {
  const FAKE_FILE = '/tmp/fake-asset.tar.gz';

  it('returns "skipped" when gh is not on PATH (default soft mode)', function() {
    // Save and clear the env var so defaults apply.
    const saved = process.env.AGENT_ANALYZER_REQUIRE_ATTESTATION;
    delete process.env.AGENT_ANALYZER_REQUIRE_ATTESTATION;
    try {
      const result = mod.verifySlsaAttestation(FAKE_FILE, {
        ghProbe: function() { return false; },
        ghRunner: function() {
          throw new Error('runner should not be called when gh is absent');
        }
      });
      assert.equal(result.status, 'skipped');
      assert.match(result.reason, /gh.*not found/i);
    } finally {
      if (saved !== undefined) process.env.AGENT_ANALYZER_REQUIRE_ATTESTATION = saved;
    }
  });

  it('returns "failed" when gh is absent and requireAttestation is true', function() {
    const result = mod.verifySlsaAttestation(FAKE_FILE, {
    ...
    assert.match(result.reason, /AGENT_ANALYZER_REQUIRE_ATTESTATION=1/);
  });

  it('honors AGENT_ANALYZER_REQUIRE_ATTESTATION=1 env var', function() {
    const saved = process.env.AGENT_ANALYZER_REQUIRE_ATTESTATION;
    ...
    try {
      ...
      if (saved === undefined) {
        ...
        process.env.AGENT_ANALYZER_REQUIRE_ATTESTATION = saved;
      }
    }
  });

  it('returns "verified" when gh runner exits 0', function() {
    let sawRepo = null;
    let sawFile = null;
    const result = mod.verifySlsaAttestation(FAKE_FILE, {
      repo: 'agent-sh/agent-analyzer',
      ghProbe: function() { return true; },
      ghRunner: function(filePath, repo) {
        sawFile = filePath;
        sawRepo = repo;
    ...
    assert.equal(sawFile, FAKE_FILE);
    assert.equal(sawRepo, 'agent-sh/agent-analyzer');
  });

  it('returns "failed" with stderr when gh runner exits non-zero', function() {
    const result = mod.verifySlsaAttestation(FAKE_FILE, {
      ghProbe: function() { return true; },
      ghRunner: function() {
        return { status: 1, stdout: '', stderr: 'no attestations found' };
      }
    });
    assert.equal(result.status, 'failed');
    assert.match(result.reason, /status 1/);
    assert.equal(result.stderr, 'no attestations found');
  });

  it('failed status is hard even when requireAttestation is false', function() {
    // Present gh + failed verify must always be failed regardless of mode.
    const result = mod.verifySlsaAttestation(FAKE_FILE, {
      requireAttestation: false,
      ghProbe: function() { return true; },
      ghRunner: function() {
        return { status: 4, stdout: '', stderr: 'signature mismatch' };
      }
    });
    assert.equal(result.status, 'failed');
  });

  it('defaults repo to GITHUB_REPO from version.js', function() {
    let sawRepo = null;
    mod.verifySlsaAttestation(FAKE_FILE, {
      ghProbe: function() { return true; },
      ghRunner: function(_filePath, repo) {
        sawRepo = repo;
...
describe('isGhAvailable', function() {
  it('returns true when custom probe returns truthy', function() {
    assert.equal(mod.isGhAvailable(function() { return true; }), true);
  });

  it('returns false when custom probe throws', function() {
    assert.equal(mod.isGhAvailable(function() { throw new Error('nope'); }), false);
  });

  it('returns false when custom probe returns falsy', function() {
    assert.equal(mod.isGhAvailable(function() { return false; }), false);
  });
});
