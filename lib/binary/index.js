 ...
 *   - The Windows zip path no longer builds a PowerShell command via string
 *     concatenation. Paths are passed as argv to `execFile`, so they never
...
async function extractTarGzToScratch(buf) {
  ...
  await new Promise(function(resolve, reject) {
    ...
    const tar = cp.spawn('tar', ['xz', '-C', tarDest], { stdio: ['pipe', 'pipe', 'pipe'] });
    let stderr = '';
    tar.stderr.on('data', function(d) { stderr += d; });
    tar.on('error', reject);
    tar.on('close', function(code) {
...
/**
 * Extract a zip buffer into a scratch directory using PowerShell Expand-Archive.
 * Paths are passed via argv (not a shell string) so they are never parsed by
 * the shell. After extraction, every resulting file is verified to be inside
 * the scratch directory (no symlink/junction escapes).
 * @param {Buffer} buf
 * @returns {Promise<string>} scratch dir
 */
async function extractZipToScratch(buf) {
  ...
  // Defense in depth: walkFiles() throws on any symlink. Also check every
  // file resolves inside scratch (guards against junctions).
