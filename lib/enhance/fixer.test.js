...
describe('applyAtPath prototype-pollution guard', () => {
  it('refuses a __proto__ segment without mutating Object.prototype', () => {
    const before = ({}).polluted;
    const out = applyAtPath({ a: 1 }, '__proto__.polluted', () => 'x');
    assert.equal(({}).polluted, before, 'Object.prototype must be untouched');
    assert.equal(out.polluted, undefined);
  });

  it('refuses constructor/prototype traversal segments', () => {
    applyAtPath({ a: 1 }, 'constructor.prototype.polluted2', () => 'x');
    assert.equal(({}).polluted2, undefined, 'no pollution via constructor.prototype');
  });

  it('still applies a normal dotted path', () => {
    const out = applyAtPath({ a: { b: 1 } }, 'a.b', (v) => v + 1);
